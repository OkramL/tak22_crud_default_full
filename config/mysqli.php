...
# 		Update: 16.05.2019 - Viimati sisestatud kirje ID ja mitme SQL lause lisamine.
# 		Update: 11.03.2023 - Translation
# 		 Fixed: 11.03.2023 - Array or string offset access with curly braces deprecated in PHP 7.4.
# PDO
...
class Db {
	...
	# Querying the database and getting the result. SELECT result as an array.
	# A command that you call in your script
	function dbGetArray($sql) {
		...
		if ($res !== false) {
			...
			if($res) {
				...
				echo '<div class="message"><strong>Viga andmebaasist k&uuml;simisega</strong></div>';
				return false;
			}
		} else {
			return false;
	...
	# Käsklus mille kutsud välja oma skriptis
	# To perform UPDATE, INSERT and DELETE commands.
	# A command that you call in your script
	function dbQuery($sql) {
		...
		if($con) {
			...
			if ($res === false) {			
				echo '<div class="message"><strong>[DEF] Vigane SQL p&auml;ring:</strong> '.$sql.'</div>';
				return false;
			}
			mysqli_close($con);
			return $res;
	...
	# Adding to the database will return false or the id of the last record
	# A command that you call in your script
	function dbQueryLastId($sql) {
		...
		if($con) {
			$res = mysqli_query($con, $sql);		
			if ($res === false) {			
				echo '<div class="message"><strong>[LST ID]Vigane SQL p&auml;ring:</strong> '.$sql.'</div>';
				return false;
			}
	...
	function dbQueryMulti($sql) {
		...
		if($con) {
			...
			if ($res === false) {			
				echo '<div class="message"><strong>[multi] Vigane SQL p&auml;ring:</strong> '.$sql.'</div>';
				return false;
			}
			mysqli_close($con);
			return $res;
	...
	# Andmebaasi lisamiseks/muutmiseks väljade varjestamiseks. Tekstilised väärtused saavad jutumärgid ümber. 
	# Numbrid mitte. 
	# Käsklus mille kutsud välja oma skriptis
	# To add/modify the database to mask the fields. Textual values are surrounded by quotation marks.
	# A command that you call in your script
	function dbFix($var) {
		if (!is_numeric($var)) {
			$var = '"'.addslashes($var).'"';
	...
	# Käsklus mille kutsud välja oma skriptis
	# To check the information received from the form
	# A command that you call in your script
	...
	# Warning: This function has been DEPRECATED as of PHP 7.4.0, and REMOVED as of 
	...
	# Käsklus mille kutsud välja oma skriptis	
	# Shows a PHP array in human form.
	# A command that you call in your script
	function show($array) {
	...
	# kujule DD.MM.YYYY
	# Changes the database date from YYYY-MM-DD to DD.MM.YYYY
	...
	# Changes the database date from YYYY-MM-DD HH:MM:SS to DD.MM.YYYY HH:MM:SS
	...
	# The clock part is removed from the database date. The shape still remains a database. 
	# Changes the database date from YYYY-MM-DD HH:MM:SS => YYYY-MM-DD
	function dbDateRemoveClock($date) {
		return date('Y-m-d', strtotime($date));
	}
	
	# Kuupäev kujul MM-DD (12-31) muudetakse kujule DD. KUUNIMI (31. Detsember)
	# A date in the format MM-DD (12-31) is changed to DD. MONTH NAME (13. Detsember)
	...
	# Date in YYYY-MM format is changed to MONTH NAME year (Jaanuar 2019)
	...
	# Moving the file name received from the form to the specified folder will also organize 
	# the file name and if such a file already exists, the sequence number will be added to it.
	...
	# This will normalize the filenames. The previous function uses it!
...
# Without this line, the preceding functions in the class are useless. The object is created 
# at the end of the class creation! A variable ($kl) that you use in your scripts to call 
# functions in the given class. For example: $kl->naita($array); Displays the contents of the 
# modifier array ($array) in human form.
$kl = new Db; # Loo objekt Db muutujasse kl (Create object Db in variable kl)
?>